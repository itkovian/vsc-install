...
def check_autogenerated_ci_config_file(testcase_instance, ci_cfg_fn, expected_contents):
    ...
    if os.path.exists(ci_cfg_ignore_fn):
        ...
        error_msg += f"from what vsc-install generates (but it's not!): {current_contents}"
        ...
        error_msg = f"Contents of {ci_cfg_fn} does not match expected contents, "
        error_msg += f"you should run 'python -m vsc.install.ci' again to re-generate {ci_cfg_fn}"
...
class CommonTest(TestCase):
    ...
    def test_prospector(self):
        ...
        self.assertFalse(failures, f"prospector failures: {pprint.pformat(failures)}")
    ...
    def test_github_actions(self):
        """Test whether github actions is in place, and was auto-generated by vsc-install."""
        vsc_ci_cfg = parse_vsc_ci_cfg()
        if os.path.exists(GITHUB_ACTIONS) or vsc_ci_cfg[ENABLE_GITHUB_ACTIONS]:
            check_autogenerated_ci_config_file(self, GITHUB_ACTIONS, gen_github_action())

    def test_ruff_toml(self):
        """Test whether ruff.toml is in place, and was auto-generated by vsc-install."""
        check_autogenerated_ci_config_file(self, RUFF_TOML, gen_ruff_toml())
